...
class Config(const.StrictModel):
    ...
    # A COMMAND that prints the restic repository password, e.g.
    # `secret get BOXYARD_RESTIC_PASSWORD`. Never the password itself: this file
    # is generated by myrig from a template and exists on every machine.
    #
    # Run ONCE per process and passed to restic in the child environment. Not
    # restic's own `--password-command`, which reruns it on every invocation --
    # `secret get` measured 0.77-1.51 s, so at one invocation per box that is
    # minutes of 1Password round trips per pass for a value that does not change.
    restic_password_command: str | None = None
