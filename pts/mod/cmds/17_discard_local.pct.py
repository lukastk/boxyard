...
# # _discard_local
#
# `boxyard discard-local` — the second way out of `WRITE_DENIED`: throw away
# this machine's local changes to a box and take the owner's copy instead.
#
# The first way out is `boxyard claim --steal`. A refusal with only one escape
# is a refusal people work around, so both are always named together.
#
# **The discarded work is not destroyed.** The force-pull runs with
# `delete_backup=False`, so everything it overwrote survives under
# `local_sync_backups_path`, and this command PRINTS that path. A command whose
# entire job is to throw work away has to be able to say where it went.

# %%
#|default_exp cmds._discard_local
#|export_as_func true
...
box_index_name = new_box(
    config_path=config_path, box_name="discard-me", storage_location=remote_name
)
...
# %% [markdown]
# ## Force-pull DATA, keeping what it overwrites
