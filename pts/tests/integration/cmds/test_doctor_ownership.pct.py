...
#|export
@pytest.mark.integration
def test_unknown_nested_config_key_reported_with_its_path(temp_boxyard):
    """
    A key inside `[storage_locations.X]` is the same trap one level down, and
    the finding has to say WHICH entry it is in -- "the config has an unknown
    key" is not actionable in a file with a dozen group tables.
    """
    remote_name, remote_rclone_path, config, config_path, data_path = temp_boxyard

    config_dict = tomllib.loads(config_path.read_text())
    config_dict["storage_locations"][remote_name]["a_key_from_the_future"] = "x"
    config_path.write_text(tomli_w.dumps(config_dict))

    findings = _findings(_doctor(config_path), "unknown-config-keys")
    assert len(findings) == 1
    assert findings[0]["unknown_keys"] == [
        f"storage_locations.{remote_name}.a_key_from_the_future"
    ]
    assert remote_name in findings[0]["message"]


# %%
#|export
@pytest.mark.integration
def test_an_unknown_nested_config_key_does_not_stop_boxyard_working(temp_boxyard):
    """
    The storage location must still be usable -- tolerance that dropped the
    whole entry would be worse than the rejection it replaces.
    """
    remote_name, remote_rclone_path, config, config_path, data_path = temp_boxyard
    ...
    assert remote_name in reloaded.storage_locations
    assert (
        reloaded.storage_locations[remote_name].store_path
        == config.storage_locations[remote_name].store_path
    )
    # And a box can still be created against it.
    idx = new_box(
