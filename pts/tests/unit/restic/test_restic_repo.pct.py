...
# %% [markdown]
# # `_restic` against a real repository
#
# These drive the actual restic binary against a LOCAL repo under `tmp_path` --
# no remote, no rclone, no network, so they are ordinary unit tests. The
# rclone-backed URL is exercised separately once `sync_box` uses it.
#
# The two traps have tests that fail if the fix is removed, in BOTH wrong
# directions where the wrong spellings differ (see `test_restic_pure.py` for the
# spelling-level pins).

# %%
#|default_exp unit.restic.test_restic_repo
...
# %% [markdown]
# ## Trap 1a: `backup --exclude` must anchor on the absolute path
#
# The DATA root's exec-bit manifest must not enter the snapshot; a manifest
# NESTED inside the box is ordinary content and must survive. A bare basename
# excludes both; a leading slash excludes neither.
...
# %% [markdown]
# ## Trap 1b: `restore --include` must anchor with a leading slash
#
# An unanchored pattern matches the basename at ANY depth, so restoring one
# changed file restores every file of that name in the box.
...
# %% [markdown]
# ## Trap 2: the snapshot records the PUSHER'S absolute path
#
# `restore <snap>:<source> --target <dir>` places the CONTENTS at the target.
# The plain form would put them under `<dir>/<absolute source path>`, so a box
# would be unrestorable onto a machine whose checkout root differs -- mymain
# alone has two.
...
# %% [markdown]
# ## Retention: a base snapshot the repo no longer holds
#
# `restic forget` on another machine may delete the snapshot this machine's
# state record names. restic's two commands disagree about how it reports that
# — `snapshots --json` exits 0 with `[]`, `backup --parent` exits 1 — so a
# returncode check turns a clean replica into a permanent false CONFLICT.
