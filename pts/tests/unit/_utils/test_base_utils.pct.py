...
# %% [markdown]
# ## Regression: unreadable directories must not be walked silently

# %%
#|export
class TestCheckLastTimeModifiedWalkErrors:
    """
    `check_last_time_modified` answers "when did this box last change?", and
    that answer drives the sync decision. It used to swallow every OSError from
    `os.scandir`, so a box with real changes underneath an unreadable directory
    reported an older mtime, looked SYNCED, and was never pushed -- data loss by
    omission with no error anywhere.

    A directory that VANISHES mid-walk is a different, legitimate race and is
    still tolerated.
    """
    ...
    def test_unreadable_directory_raises(self, tmp_path):
        ...
        try:
            with pytest.raises(OSError) as exc:
                check_last_time_modified(tmp_path)
            # The message must name the directory that has to be fixed.
            assert "locked" in str(exc.value)
        finally:
            os.chmod(locked, 0o755)

    def test_file_vanishing_between_scandir_and_stat_is_tolerated(self, tmp_path):
        import os
        ...
        with patch.object(os.DirEntry, "stat", flaky_stat):
            result = check_last_time_modified(tmp_path)
        assert result is not None, "a vanished file should be skipped, not fatal"

    def test_missing_directory_entry_is_tolerated(self, tmp_path):
        """A subdirectory removed between being queued and being scanned."""
        import os
        from unittest.mock import patch
    ...
    def test_normal_tree_still_reports_the_newest_file(self, tmp_path):
        import time
